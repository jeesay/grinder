...
do_raw       "Import raw movies/micrographs?", true, "Set this to Yes if you plan to import raw movies or micrographs"
...
Cs  "Spherical aberration (mm):" range "2.7, 0, 8, 0.1"   "Spherical aberration of the microscope used to collect these images (in mm). Typical values are 2.7 (FEI Titan & Talos, most JEOL CRYO-ARM), 2.0 (FEI Polara), 1.4 (some JEOL CRYO-ARM) and 0.01 (microscopes with a Cs corrector)."
Q0  "Amplitude contrast:"        range "0.1, 0, 0.3, 0.01" "Fraction of amplitude contrast. Often values around 10% work better than theoretically more accurate lower values..."
...
_cli.bool
prog    "relion_import"
io      '--i ${fn_in_raw} --do_movies --odir ${outputname}--o movies.star' ? ?
param   " --optics_group_name ${optics_group}" ? ?
flag    " --optics_group_mtf ${fn_mtf}" ? true 
param   " --angpix ${angpix} --kV ${kV} --Cs ${Cs} --Q0 ${Q0} --beamtilt_x ${beamtilt_x} --beamtilt_y ${beamtilt_y}" ? ?

data_coords
#

"do_other"  "Import other node types?", false, "Set this to Yes  if you plan to import anything else than movies or micrographs");

"fn_in_other"  "Input file:", "ref.mrc", "Input file (*.*)", ".", "Select any file(s) to import. \n \n \
...
"node_type"  "Node type:", job_nodetype_options, 0, "Select the type of Node this is.");
"optics_group_particles"  "Rename optics group for particles:", (std::string)"", "Only for the import of a particles STAR file with a single, or no, optics groups defined: rename the optics group for the imported particles to this string.");
}
		...
		std::string &final_command, bool do_makedir, int job_counter, std::string &error_message)
{
	...
	if (do_raw)
	{
		...
		{
			command += " --optics_group_mtf ${fn_mtf"].getString();
		}
		command += " --angpix ${angpix} --kV ${kV} --Cs ${Cs} --Q0 ${Q0} --beamtilt_x ${beamtilt_x} --beamtilt_y ${beamtilt_y"].getString();
