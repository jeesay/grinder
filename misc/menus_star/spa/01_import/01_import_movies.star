...
_io.default
_io.help
input         bi-arrow-bar-down 'Input' fieldset ? ?
#
loop_
_settings.id
_settings.icon
...
_settings.default
_settings.help
general       ? 'General' fieldset ? ?
other         ? 'Additional Parameters' fieldset ? ?
#
loop_
_compute.id
_compute.icon
...
_compute.default
_compute.help
disk          ? 'Disk Access' fiedset ?
use_gpu       ? 'Use GPU Acceleration?' switch false 'If set to Yes, the job will try to use GPU acceleration.'
#
loop_
_running.id
_running.icon
...
_running.help
process       ? ? fieldset ? ?
do_queue      ? 'Submit to queue?' switch false 'If set to Yes, the job will be submit to a queue, otherwise the job will be executed locally. Note that only MPI jobs may be sent to a queue. The default can be set through the environment variable RELION_QUEUE_USE.''
#
# Command Options
loop_
_input.id
...
io       '--o Import/job{RELION_ID:03}/movies.star' ? ?
